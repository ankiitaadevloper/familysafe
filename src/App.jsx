...
const Icon = ({ name, size = 20 }) => {
  const icons = {
    ...
    car: <><path d="M5 17h14M7 17v-4l2-6h6l2 6v4M7 13h10" stroke="currentColor" strokeWidth="1.8" fill="none" strokeLinejoin="round"/><circle cx="8" cy="18" r="1.5" fill="currentColor"/><circle cx="16" cy="18" r="1.5" fill="currentColor"/></>,
    ...
    arrowLeft: <path d="M15 18l-6-6 6-6" stroke="currentColor" strokeWidth="1.8" fill="none" strokeLinecap="round" strokeLinejoin="round"/>,
    ...
    battery: <><rect x="2" y="7" width="18" height="10" rx="2" stroke="currentColor" strokeWidth="1.8" fill="none"/><line x1="22" y1="11" x2="22" y2="13" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></>,
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', padding:'32px 24px', background:'radial-gradient(ellipse 600px 600px at 50% 0%, rgba(107,168,255,0.08), transparent)' }}>
      ...
      <h1 style={{ fontFamily:'Fraunces, serif', fontSize:52, lineHeight:0.95, fontWeight:500, letterSpacing:'-0.04em', marginBottom:20, animation:'fadeUp 0.8s ease-out' }}>
        Drive safe.<br/>
        Stay <em style={{ fontStyle:'italic', color:'#d4a574', fontWeight:400 }}>connected.</em>
      </h1>
...
{error && (
          <div style={{ padding:'10px 14px', background:'rgba(255,94,108,0.1)', border:'1px solid rgba(255,94,108,0.3)', borderRadius:10, fontSize:13, color:'#ff5e6c', textAlign:'center', marginTop:4 }}>
            {error}
          </div>
        )}
        <button
          ...
          style={{ width:'100%', padding:18, borderRadius:14, background: loading ? '#ccc' : '#f5f5f0', color:'#0a0e1a', border:'none', fontFamily:'inherit', fontSize:15, fontWeight:600, cursor: loading ? 'not-allowed' : 'pointer', marginTop:8, transition:'all 0.2s' }}
        >
...
// ===== DASHBOARD =====
  ...
  useEffect(() => {
    ...
    const map = L.map(mapRef.current, { center:[featured.location.lat, featured.location.lng], zoom:11, zoomControl:false })
  ...
  useEffect(() => {
    ...
    // If YOUR journey is active, show YOUR real location
    if (isJourneyActive && myLocation) {
      const carIcon = L.divIcon({ html:'<div class="custom-car-icon"><svg width="18" height="18" viewBox="0 0 24 24"><path d="M5 17h14M7 17v-4l2-6h6l2 6v4M7 13h10" stroke="#0a0e1a" stroke-width="1.5" fill="none"/><circle cx="8" cy="18" r="1.3" fill="#0a0e1a"/><circle cx="16" cy="18" r="1.3" fill="#0a0e1a"/></svg></div>', iconSize:[32,32], iconAnchor:[16,16], className:'' })
      markersRef.current.push(L.marker([myLocation.lat, myLocation.lng],{icon:carIcon}).addTo(map))
      map.setView([myLocation.lat, myLocation.lng], 14)
      return
    }
    if (featured.isDriving) {
      const route = featured.id==='bro'
        ? [[MUMBAI.lat,MUMBAI.lng],[LONAVALA.lat,LONAVALA.lng],[featured.location.lat,featured.location.lng],[HOME.lat,HOME.lng]]
        : [[HOME.lat,HOME.lng],[featured.location.lat,featured.location.lng]]
      markersRef.current.push(L.polyline(route,{color:'#d4a574',weight:3,opacity:0.8,dashArray:'6,6'}).addTo(map))
      const carIcon = L.divIcon({ html:'<div class="custom-car-icon"><svg width="18" height="18" viewBox="0 0 24 24"><path d="M5 17h14M7 17v-4l2-6h6l2 6v4M7 13h10" stroke="#0a0e1a" stroke-width="1.5" fill="none"/><circle cx="8" cy="18" r="1.3" fill="#0a0e1a"/><circle cx="16" cy="18" r="1.3" fill="#0a0e1a"/></svg></div>', iconSize:[32,32], iconAnchor:[16,16], className:'' })
      markersRef.current.push(L.marker([featured.location.lat,featured.location.lng],{icon:carIcon}).addTo(map))
      featured.id==='bro' ? map.fitBounds([[MUMBAI.lat,MUMBAI.lng],[HOME.lat,HOME.lng]],{padding:[20,20]}) : map.setView([featured.location.lat,featured.location.lng],13)
    } else {
      map.setView([HOME.lat,HOME.lng],12)
    }
  }, [featured.id, featured.location.lat, featured.location.lng, featured.isDriving])
    ...
    <div style={S.app}>
      <div style={S.topBar}>
        ...
        <div style={{ display:'flex', gap:10 }}>
          <button style={S.iconBtn} onClick={() => goTo('alerts')}>
      ...
      <div style={{ padding:'0 20px 20px', display:'flex', gap:14, overflowX:'auto', scrollbarWidth:'none' }}>
        {members.map(m => (
          <div key={m.id} onClick={() => onSelectMember(m.id===selectedId ? null : m.id)} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:8, cursor:'pointer', flexShrink:0, minWidth:60 }}>
            <div style={{ width:58, height:58, borderRadius:'50%', position:'relative', padding: m.id===selectedId ? 2.5 : 2, background: m.id===selectedId ? 'linear-gradient(135deg,#d4a574,#b88a5c)' : 'rgba(31,38,64,0.55)', border:'1px solid rgba(255,255,255,0.08)', transition:'all 0.2s' }}>
              <div style={{ width:'100%', height:'100%', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:600, fontSize:18, color:'#0a0e1a', background: m.avatar==='avatar-mom' ? 'linear-gradient(135deg,#f9c9b6,#e89570)' : m.avatar==='avatar-dad' ? 'linear-gradient(135deg,#b6dcf9,#70a8e8)' : m.avatar==='avatar-sis' ? 'linear-gradient(135deg,#d9b6f9,#a570e8)' : m.avatar==='avatar-bro' ? 'linear-gradient(135deg,#f9eab6,#e8c870)' : 'linear-gradient(135deg,#b6f9d9,#70e8a5)' }}>
                {m.initial}
              </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:8, cursor:'pointer', flexShrink:0, minWidth:60 }}>
      ...
      <div style={{ margin:'0 20px 16px', ...S.glass, borderRadius:22, padding:20, position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:18 }}>
          <div style={{ width:46, height:46, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:600, fontSize:18, color:'#0a0e1a', flexShrink:0, background: featured.avatar==='avatar-mom' ? 'linear-gradient(135deg,#f9c9b6,#e89570)' : featured.avatar==='avatar-dad' ? 'linear-gradient(135deg,#b6dcf9,#70a8e8)' : featured.avatar==='avatar-sis' ? 'linear-gradient(135deg,#d9b6f9,#a570e8)' : featured.avatar==='avatar-bro' ? 'linear-gradient(135deg,#f9eab6,#e8c870)' : 'linear-gradient(135deg,#b6f9d9,#70e8a5)' }}>
            {featured.initial}
          </div>
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ fontSize:12, color:'#a4a8b8', display:'flex', alignItems:'center', gap:4 }}>
              ...
              {isJourneyActive && myLocation
                ...
                : featured.isDriving
                  ? `${featured.routeFrom} → ${featured.routeTo}`
                  : 'At home'}
            </div>
          </div>
          <div style={{ padding:'6px 12px', borderRadius:100, fontSize:11, fontWeight:600, fontFamily:'Geist Mono, monospace', textTransform:'uppercase', letterSpacing:'0.05em', display:'flex', alignItems:'center', gap:5, background: featured.status==='warn' ? 'rgba(255,179,71,0.15)' : featured.isDriving ? 'rgba(107,168,255,0.15)' : 'rgba(94,221,180,0.15)', color: featured.status==='warn' ? '#ffb347' : featured.isDriving ? '#6ba8ff' : '#5eddb4', border: `1px solid ${featured.status==='warn' ? 'rgba(255,179,71,0.3)' : featured.isDriving ? 'rgba(107,168,255,0.3)' : 'rgba(94,221,180,0.3)'}` }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr auto', gap:16, alignItems:'center', padding:'14px 0', borderTop:'1px solid rgba(255,255,255,0.06)', borderBottom:'1px solid rgba(255,255,255,0.06)', marginBottom:14 }}>
          <div>
            ...
            <div style={{ fontFamily:'Fraunces, serif', fontSize:22, fontWeight:600, letterSpacing:'-0.02em' }}>{Math.round(featured.speed)}<span style={{ fontSize:11, color:'#a4a8b8', marginLeft:2 }}>km/h</span></div>
          </div>
          <div>
            <div style={{ fontFamily:'Geist Mono, monospace', fontSize:10, textTransform:'uppercase', letterSpacing:'0.08em', color:'#6b7088', marginBottom:4 }}>ETA</div>
            <div style={{ fontFamily:'Fraunces, serif', fontSize:22, fontWeight:600, letterSpacing:'-0.02em' }}>{featured.isDriving ? featured.eta : '—'}</div>
          </div>
          <div style={{ position:'relative', width:110, height:70 }}>
            <Gauge speed={featured.speed} compact/>
            <div style={{ position:'absolute', top:26, left:0, right:0, textAlign:'center' }}>
              <div style={{ fontFamily:'Fraunces, serif', fontSize:22, fontWeight:700, letterSpacing:'-0.02em', lineHeight:1 }}>{Math.round(featured.speed)}</div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginTop:14 }}>
          {[
            { label:'Distance', value: featured.isDriving ? `${featured.tripDistance?.toFixed(1)}` : '0.0', unit:'km' },
            { label:'Top Speed', value: featured.isDriving ? featured.topSpeed : '0', unit:'km/h' },
  ...
  return (
    <div style={{ ...S.app, paddingBottom:90 }}>
      ...
      <div style={{ padding:'0 20px', marginBottom:24 }}>
        ...
        <div style={{ ...S.glass, borderRadius:16, overflow:'hidden' }}>
          {members.filter(m=>m.id!=='me').map((m, i, arr) => (
            <div key={m.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'14px 16px', borderBottom: i<arr.length-1 ? '1px solid rgba(255,255,255,0.06)' : 'none' }}>
              <div style={{ width:36, height:36, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:600, fontSize:14, color:'#0a0e1a', flexShrink:0, background: m.avatar==='avatar-mom' ? 'linear-gradient(135deg,#f9c9b6,#e89570)' : m.avatar==='avatar-dad' ? 'linear-gradient(135deg,#b6dcf9,#70a8e8)' : m.avatar==='avatar-sis' ? 'linear-gradient(135deg,#d9b6f9,#a570e8)' : 'linear-gradient(135deg,#f9eab6,#e8c870)' }}>
                {m.initial}
              </div>
              <div style={{ flex:1, minWidth:0 }}>
  ...
  <div onClick={(e)=>e.target===e.currentTarget&&onDismiss()} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.75)', backdropFilter:'blur(8px)', zIndex:1000, display:'flex', alignItems:'flex-end', justifyContent:'center', animation:'fadeIn 0.3s ease' }}>
    <div style={{ background:'#1a2033', border:'1px solid rgba(255,94,108,0.4)', borderRadius:'24px 24px 0 0', padding:32, width:'100%', maxWidth:480, animation:'slideUp 0.4s cubic-bezier(0.16,1,0.3,1)', position:'relative' }}>
      ...
      <p style={{ color:'#a4a8b8', textAlign:'center', fontSize:14, lineHeight:1.5, marginBottom:20 }}>
        {isSOS ? 'Your live location has been shared with all family members.' : `Was driving at 68 km/h, then stopped suddenly. Stationary for over 3 minutes.`}
      </p>
      <div style={{ background:'#131826', border:'1px solid rgba(255,255,255,0.06)', borderRadius:14, padding:'4px 16px', marginBottom:20 }}>
        {[
          { label: isSOS ? 'Current speed' : 'Last speed', value: `${Math.round(member.speed)||68} km/h`, color:'#ffb347' },
          { label:'Location', value:'Mumbai-Pune Hwy', color:'#f5f5f0' },
          { label:'Distance from you', value:`${member.distance?.toFixed(1)||'2.4'} km`, color:'#f5f5f0' },
        ].map(row => (
...
export default function App() {
  ...
  const startTracking = async () => {
    ...
    // Ask permission and start watching
    watchIdRef.current = navigator.geolocation.watchPosition(
      async (position) => {
        ...
        // Check overspeed
        if (speedKmh > 80) {
          await setDoc(doc(db, 'alerts', `overspeed-${auth.currentUser.uid}`), {
            type: 'overspeed',
            speed: speedKmh,
            uid: auth.currentUser.uid,
            timestamp: serverTimestamp()
  ...
  useEffect(() => {
    members.forEach(m => {
      if (m.status==='warn' && !lastAlertedRef.current[m.id]) {
        lastAlertedRef.current[m.id] = Date.now()
        setAlerts(prev => [{
          id:`warn-${m.id}-${Date.now()}`, memberId:m.id, severity:'warn',
          type:'Overspeed Alert', title:`${m.name} is driving fast`,
          time:'Just now', speed:`${Math.round(m.speed)} km/h`,
          location:'On the road', desc:`Currently at ${Math.round(m.speed)} km/h — over the 80 km/h limit.`
        }, ...prev])
      }
      if (m.status!=='warn') delete lastAlertedRef.current[m.id]
    })
  }, [members])

  const triggerAccident = (memberId) => {
    const m = members.find(x=>x.id===memberId)
    if (!m) return
    setMembers(prev => prev.map(x => x.id===memberId ? {...x,speed:0,status:'danger'} : x))
    setModal({ ...m, speed:0, lastSpeed:68 })
    setAlerts(prev => [{ id:`danger-${memberId}-${Date.now()}`, memberId, severity:'danger', type:'Accident Detection', title:`🚨 ${m.name} stopped suddenly`, time:'Just now', speed:'68 km/h (before stop)', location:'Mumbai-Pune Hwy', desc:`Was at 68 km/h, no movement for 3+ min — possible accident.` }, ...prev])
  }

  const triggerSOS = () => {
    setIsSOS(true)
    setModal(members.find(m=>m.id==='me'))
  ...
  return (
    <>
      ...
      {modal && (
        <Modal
          member={modal} isSOS={isSOS}
          onDismiss={() => { setModal(null); setIsSOS(false) }}
          onConfirmOk={() => {
            setModal(null); setIsSOS(false)
